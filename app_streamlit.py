...
def _normalize_client_series(s: pd.Series) -> pd.Series:
    """Normaliza Client para que NO se repita por formato.
    Ejemplos que se vuelven iguales: ' 74758', '74758 ', '74758\r' -> '74758'
    También elimina caracteres no numéricos.
    """
    s = s.fillna('').astype(str).str.strip()
    s = s.apply(lambda v: ''.join([c for c in v if c.isdigit()]))
    return s
...
if uploaded:
    with tempfile.TemporaryDirectory() as td:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(['🟡 Pares similares', '✅ Exactos', '🧾 Clientes únicos', '📨 Enviar a preventivos'])

        with tab1:
            st.subheader('🟡 Pares similares')
            st.dataframe(df_sim_f.sort_values(['prioridad'], ascending=False) if not df_sim_f.empty and 'prioridad' in df_sim_f.columns else df_sim_f,
        ...
        with tab2:
            ...
            st.dataframe(df_show.sort_values(['prioridad'], ascending=False) if not df_show.empty and 'prioridad' in df_show.columns else df_show,
                         use_container_width=True, hide_index=True)
        ...
        with tab3:
            ...
            st.caption('Acá NO se repite el número de cliente. Se normaliza a solo dígitos.')
            st.dataframe(df_clients_sum, use_container_width=True, hide_index=True)
            st.download_button('Descargar lista_clientes_unicos.csv', data=df_clients_sum.to_csv(index=False).encode('utf-8'), file_name='lista_clientes_unicos.csv', mime='text/csv')
        ...
        with tab4:
            ...
            if not df_sel.empty:
                ...
                st.download_button('Descargar mensaje_preventivos.txt', data=mensaje.encode('utf-8'), file_name='mensaje_preventivos.txt', mime='text/plain')
